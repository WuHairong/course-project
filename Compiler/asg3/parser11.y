...
fielddecl: TOK_BOOL  TOK_IDENT       {swap($2,TOK_FIELD) ;$$ = adopt1 ($1, $2);}
         ...
         | TOK_IDENT TOK_IDENT       {swap($2,TOK_FIELD) ;$$ = adopt1 ($1, $2);}
         | basetype TOK_ARRAY TOK_IDENT   {swap($3, TOK_FIELD);$$ = adopt2 ($2, $1, $3);}	
         ;
basetype : TOK_VOID                  {$$ = $1;}
         | TOK_BOOL                  {$$ = $1;}
         | TOK_CHAR                  {$$ = $1;}
...
identdecl: basetype TOK_IDENT        {swap($2, TOK_DECLID); $$ = adopt1 ($1, $2);}
         | basetype TOK_ARRAY TOK_IDENT  {swap($3, TOK_DECLID); $$ = adopt2 ($2, $1, $3);} 
         ;
...
expr     : expr '=' expr             { $$ = adopt2 ($2, $1, $3); }
         ...
         | constant                  { $$ = $1;                  }
         ;
allocator: TOK_NEW TOK_IDENT '('')'      {swap($2, TOK_TYPEID);free_ast2($3,$4);
                                          $$ = adopt1($1, $2);}
         | TOK_NEW TOK_STRING '('expr')' {swap($2, TOK_NEWSTRING);
                                          free_ast3($1,$3, $5);
                                          $$ = adopt1 ($2, $4); } 
         | TOK_NEW basetype '[' expr ']' {swap($1, TOK_NEWARRAY);
                                          swap($2, TOK_TYPEID);free_ast2($3,$5);
                                          $$ = adopt2 ($1, $2, $4); }
         ;
call     : callb ')'              {free_ast($2); $$= $1;                      }
         | TOK_IDENT '('')'       {free_ast($3);swap($2, TOK_CALL);
                                   $$ = adopt1($2,$1);    }
         ;
