...
allocator: TOK_NEW TOK_IDENT '('')'  {swap($2, TOK_TYPEID);free_ast2($3,$4);
         ...
         | TOK_NEW TOK_STRING '('expr')' {swap($2, TOK_NEWSTRING);
                                          free_ast3($1,$3, $5);
                                          $$ = adopt1 ($2, $4); } 
         | TOK_NEW TOK_INT '[' expr ']'  {swap($1, TOK_NEWARRAY);
                                          swap($2, TOK_TYPEID);free_ast2($3,$5);
                                          $$ = adopt2 ($1, $2, $4); }
         |TOK_NEW TOK_CHAR '[' expr ']'  {swap($1, TOK_NEWARRAY);
                                          swap($2, TOK_TYPEID);free_ast2($3,$5);
                                          $$ = adopt2 ($1, $2, $4); }
         | TOK_NEW TOK_BOOL '[' expr ']'  {swap($1, TOK_NEWARRAY);
                                          swap($2, TOK_TYPEID);free_ast2($3,$5);
                                          $$ = adopt2 ($1, $2, $4); }
         ;
call     : callb ')'              {free_ast($2); $$= $1;                      }
         ;
callb    : TOK_IDENT '('          {swap($2, TOK_CALL);$$ = adopt1($2, $1); }
         | TOK_IDENT '('expr      {swap($2, TOK_CALL);$$ = adopt2($2, $1,$3); }
